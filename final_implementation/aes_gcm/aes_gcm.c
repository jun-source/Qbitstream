	...
	unsigned char *ciphertext, unsigned char *tag)
{
	...
	if(!(ctx = EVP_CIPHER_CTX_new()))
		error_aes("[symmetric crypto]: encryption failed");
	...
	if(1 != EVP_EncryptInit_ex(ctx, NULL, NULL, key, iv)) error_aes("[symmetric crypto]: encryption failed");
	 ...
	 */
	if(1 != EVP_EncryptFinal_ex(ctx, ciphertext + ciphertext_len, &len)) error_aes("[symmetric crypto]: encryption failed");
	ciphertext_len += len;
	...
	unsigned char *plaintext)
{
	...
	/* Create and initialise the context */
	if(!(ctx = EVP_CIPHER_CTX_new())) 
		error_aes("[symmetric crypto]: decryption failed");
	...
	/* Initialise key and IV */
	if(!EVP_DecryptInit_ex(ctx, NULL, NULL, key, iv)) error_aes("[symmetric crypto]: decryption failed");
