...
int decrypt_0(unsigned char *ciphertext, int ciphertext_len, unsigned char *aad,
	int aad_len, unsigned char *tag, unsigned char *key, unsigned char *iv,
	unsigned char *plaintext)
{
	EVP_CIPHER_CTX *ctx;
...
int main (int argc, char **argv)
{
	...
	/* automatic generation of random key */
	...
	k = decrypt_0(ciphertext, k, aad, sizeof(aad), tag, key, iv, pt);
	if(k>0)
	{
		pt[k]='\0';
		printf("%s\n",pt);
